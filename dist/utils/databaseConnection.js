...
class DatabaseConnectionService {
     ...
     */
    static extractDatabaseName(connectionString, dbType) {
        try {
            switch (dbType) {
                case 'postgresql':
     ...
     */
    static async testConnection(encryptedConnectionString, dbType) {
        try {
            const connectionString = encryption_1.EncryptionService.decrypt(encryptedConnectionString);
     ...
     * Get database schema
     */
    static async getSchema(encryptedConnectionString, dbType) {
        const connectionString = encryption_1.EncryptionService.decrypt(encryptedConnectionString);
     ...
     * Execute SQL query
     */
    static async executeQuery(encryptedConnectionString, dbType, query) {
        const connectionString = encryption_1.EncryptionService.decrypt(encryptedConnectionString);
        try {
            const connection = await this.createConnection(connectionString, dbType);
     ...
     */
    static async createConnection(connectionString, dbType) {
        switch (dbType) {
            case 'postgresql':
                ...
                return pgClient;
            case 'mysql':
                return await promise_1.default.createConnection(connectionString);
            case 'mongodb':
                const mongoClient = new mongodb_1.MongoClient(connectionString);
                await mongoClient.connect();
                return mongoClient;
            case 'sqlserver':
                return await mssql_1.default.connect(connectionString);
            case 'sqlite':
     ...
     */
    static async closeConnection(connection, dbType) {
        try {
            switch (dbType) {
                case 'postgresql':
     ...
     */
    static async getTables(connection, dbType) {
        switch (dbType) {
            case 'postgresql':
     ...
     */
    static async runQuery(connection, dbType, query) {
        switch (dbType) {
            case 'postgresql':
    ...
    static async getPostgreSQLTables(connection) {
        const query = `
      SELECT 
        t.table_name,
        c.column_name,
     ...
     */
    static async getPostgreSQLEnumValues(connection, columnName) {
        try {
    ...
    static async getMySQLTables(connection) {
    ...
    `;
        ...
        const tables = tablesResult.map(row => row.table_name).filter(name => name != null);
        // console.log('Found tables:', tables);
        const tableInfos = [];
        for (const tableName of tables) {
            if (!tableName)
                continue; // Skip undefined/null table names
    ...
    static async getMongoDBCollections(connection) {
     ...
     */
    static async executeMongoDBQuery(connection, query) {
        try {
    ...
    static async getSQLServerTables(connection) {
        const query = `
      SELECT 
        t.table_name,
        c.column_name,
    ...
    static async getSQLiteTables(connection) {
     ...
     */
    static groupColumnsByTable(rows) {
...
DatabaseConnectionService.connections = new Map();
