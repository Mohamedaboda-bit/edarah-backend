...
exports.RAGService = exports.userMemories = void 0;
...
exports.userMemories = userMemories;
const MAX_HISTORY = 10;
function getUserMemory(userId) {
    if (!userMemories.has(userId)) {
        userMemories.set(userId, new memory_1.BufferMemory({
            returnMessages: true,
...
async function saveConversation(userId, question, response) {
    try {
        const memory = getUserMemory(userId);
        await memory.saveContext({ question }, { response });
...
async function classifyQuestion(question, chatHistory) {
    try {
        ...
        const result = await (0, langchain_1.executeQuestionClassification)({
            question,
            chatHistory
        });
        ...
        if (typeof classification.needsDatabase !== 'boolean') {
            console.warn('Invalid needsDatabase value, defaulting to database');
            classification.needsDatabase = true;
        }
...
class RAGService {
     ...
     */
    static async processGeneralRequest(request, chatHistory) {
        try {
            console.log('Processing general knowledge request for question:', request.question);
            const response = await (0, langchain_1.executeGeneralKnowledge)({
     ...
     */
    static async processDatabaseRequest(request, chatHistory) {
        try {
            ...
            const prompt = langchain_1.PROMPT_TEMPLATES.formatSQLPrompt({
            ...
            let sqlResult = await langchain_1.deepSeekCodingLLM.call({ prompt });
            ...
            try {
                ...
                sqlResult = await langchain_1.deepSeekCodingLLM.call({ prompt: fixPrompt });
            ...
            const documents = await vectorStore_1.VectorStoreService.processQueryResults(request.userId, databaseInfo.id, queryResult, request.question, sqlQuery);
            try {
                await vectorStore_1.VectorStoreService.addDocuments(request.userId, databaseInfo.id, documents.map(doc => ({
            ...
            let relevantContext = '';
            try {
                relevantContext = await vectorStore_1.VectorStoreService.getRelevantContext(request.userId, databaseInfo.id, request.question);
            }
            catch (error) {
            ...
            const businessPrompt = langchain_1.PROMPT_TEMPLATES.formatBusinessAnalysisPrompt({
                question: request.question,
                data: dataContext,
            ...
            const insightsResult = await langchain_1.deepSeekChatLLM.call({ prompt: businessPrompt });
     ...
     */
    static async getUserDatabase(userId, databaseId) {
        try {
            let userDatabase;
            if (databaseId) {
                // Get specific database
                userDatabase = await index_1.prisma.user_databases.findFirst({
                    where: {
     ...
     */
    static async getSchema(databaseInfo) {
        try {
            // Check if we have a cached schema that's not too old (weekly refresh)
            const userDatabase = await index_1.prisma.user_databases.findUnique({
            ...
            if (userDatabase?.schema_cache) {
                ...
                if (Date.now() - lastUpdate.getTime() < weekInMs) {
                    // Use cached schema
                    return userDatabase.schema_cache;
                }
            }
            // Get fresh schema
            const schema = await databaseConnection_1.DatabaseConnectionService.getSchema(databaseInfo.connectionString, databaseInfo.type);
            // Update cache
     ...
     */
    static async generateSQLQuery(question, schema) {
        try {
            const schemaDescription = this.formatSchemaForPrompt(schema);
            const result = await (0, langchain_1.executeSQLGeneration)({
                schema: schemaDescription,
                question: question,
                databaseType: schema.databaseType
            });
     ...
     * Execute SQL query
     */
    static async executeQuery(databaseInfo, sqlQuery) {
        try {
            console.log('Executing query:', sqlQuery);
            console.log('Database info:', { type: databaseInfo.type, name: databaseInfo.name });
            const result = await databaseConnection_1.DatabaseConnectionService.executeQuery(databaseInfo.connectionString, databaseInfo.type, sqlQuery);
     ...
     */
    static async generateInsights(question, data, context, vectorContext) {
        try {
            const dataContext = this.formatDataForPrompt(data);
            const combinedContext = [context, vectorContext].filter(Boolean).join('\n\n');
            const result = await (0, langchain_1.executeBusinessAnalysis)({
                question: question,
                data: dataContext,
                context: combinedContext || 'No additional context provided'
            });
     ...
     */
    static formatSchemaForPrompt(schema) {
     ...
     */
    static formatDataForPrompt(data) {
     ...
     */
    static fixPostgreSQLTableNames(query, schema) {
     ...
     */
    static fixPostgreSQLEnumComparisons(query, schema) {
        let fixedQuery = query;
        // Find columns that are USER-DEFINED (enums)
        const enumColumns = schema.tables.flatMap(table => table.columns
     ...
     */
    static validateAndFixSQLSyntax(query) {
        let fixedQuery = query;
     ...
     * Remove WHERE conditions that might filter out all data
     */
    static removeProblematicWhereConditions(query) {
        let fixedQuery = query;
     ...
     */
    static generateSimpleFallbackQuery(schema, question) {
        // Find tables with financial/revenue data
        const financialTables = schema.tables.filter(table => table.columns.some(col => col.name.toLowerCase().includes('earned') ||
        ...
        if (financialTables.length > 0) {
            ...
            const financialColumns = table.columns.filter(col => col.type === 'integer' || col.type === 'numeric' || col.type === 'bigint');
        ...
        const anyTable = schema.tables.find(table => table.columns.some(col => col.type === 'integer' || col.type === 'numeric' || col.type === 'bigint'));
        if (anyTable) {
            const numericColumn = anyTable.columns.find(col => col.type === 'integer' || col.type === 'numeric' || col.type === 'bigint');
            if (numericColumn) {
     ...
     */
    static async analyzeBusinessType(schema) {
        try {
            const schemaDescription = this.formatSchemaForPrompt(schema);
            const result = await (0, langchain_1.executeSchemaAnalysis)({
                schema: schemaDescription
            });
            if (!result) {
     ...
     */
    static async getHistoricalContext(userId, databaseId, question) {
        try {
            return await vectorStore_1.VectorStoreService.getRelevantContext(userId, databaseId, question, 5);
        }
        catch (error) {
     ...
     */
    static async clearUserVectorStore(userId, databaseId) {
        try {
