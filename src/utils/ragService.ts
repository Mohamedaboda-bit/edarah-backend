...
export { getUserMemory, clearUserMemory, userMemories };
...
export class RAGService {
  ...
  static async processRequest(request: RAGRequest): Promise<RAGResponse> {
    try {
      ...
      // Step 3: Get user's memory
      const memory = getUserMemory(request.userId);
      ...
      if (schema.databaseType === 'mongodb') {
        ...
        const sqlMatch = sqlQuery.match(/(SELECT|INSERT|UPDATE|DELETE)[\s\S]*/i);
        if (sqlMatch) {
      ...
      try {
        ...
        if (schema.databaseType === 'mongodb') {
          ...
          const sqlMatch2 = sqlQuery.match(/(SELECT|INSERT|UPDATE|DELETE)[\s\S]*/i);
          if (sqlMatch2) {
      ...
      let insights: any = {};
      try {
        insights = JSON.parse(insightsResult.text || '{}');
      } catch {
        insights = {
