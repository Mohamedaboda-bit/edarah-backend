  ...
  private static async executeMongoDBQuery(connection: MongoClient, query: string): Promise<any[]> {
    try {
      ...
      try {
        ...
        const normalizedPipeline = pipelineStr
          .replace(/'/g, '"')
        ...
        throw new Error(`Failed to parse MongoDB aggregation pipeline: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`);
      }
